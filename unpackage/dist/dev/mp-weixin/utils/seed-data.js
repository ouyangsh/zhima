...
const seedDiscoverData = async () => {
  try {
    const uid = common_vendor.tr.getCurrentUserInfo().uid;
    if (!uid)
      return;
    const postsCount = await db.collection("knitting-posts").count();
    if (postsCount.result.total === 0) {
      common_vendor.index.__f__("log", "at utils/seed-data.js:11", "Seeding posts...");
      const samplePosts = [
        {
          title: "彩虹毯子",
      ...
      common_vendor.index.__f__("log", "at utils/seed-data.js:50", "Posts seeded.");
    }
    const activitiesCount = await db.collection("knitting-activities").where({ user_id: uid }).count();
    if (activitiesCount.result.total === 0) {
      common_vendor.index.__f__("log", "at utils/seed-data.js:56", "Seeding activities...");
      ...
      common_vendor.index.__f__("log", "at utils/seed-data.js:98", "Activities seeded.");
    }
    return true;
  } catch (e) {
    common_vendor.index.__f__("error", "at utils/seed-data.js:103", "Seeding failed:", e);
    return false;
  }
};
exports.seedDiscoverData = seedDiscoverData;
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/seed-data.js.map
